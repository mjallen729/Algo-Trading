...
class SuperAlgorithm:
  ...
  async def _train_new_models(self):
    ...
    try:
      ...
      if primary_asset in training_data:
        ...
        logger.error(
          f"Primary asset {primary_asset} not available for training")
  ...
  async def _process_asset(self, asset: str):
    ...
    try:
      ...
      logger.info(
        f"{asset} - Prediction: {prediction:.2f} (confidence: {confidence:.3f})")
  ...
  async def start(self):
    ...
    # Main trading loop
    cycle_interval = config.get(
      'trading.cycle_interval', 300)  # 5 minutes default
