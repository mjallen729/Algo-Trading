...
class DataLoader:
    ...
    def load_local_csv(self, symbol: str) -> pd.DataFrame:
        ...
        if not os.path.exists(file_path):
            ...
            file_path = os.path.join(DATA_DIR, "raw", f"{symbol.lower()}_2020-01-16_2025-01-14.csv")
            if not os.path.exists(file_path):
    ...
    def get_data(self, symbol: str, start_date: datetime, end_date: datetime, use_local: bool = False) -> pd.DataFrame:
        if use_local:
            ...
            print(f"Loading historical data from Alpaca for {symbol} from {start_date} to {end_date}...")
