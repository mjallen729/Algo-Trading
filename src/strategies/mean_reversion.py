...
class MeanReversionStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict = None):
        ...
        logger.info(f"MeanReversionStrategy initialized with Bollinger std: {self.bollinger_std}")
    ...
    def _add_mean_reversion_indicators(self, indicators: pd.DataFrame) -> pd.DataFrame:
        ...
        indicators['Price_MA'] = indicators['Close'].rolling(self.config['mean_reversion_period']).mean()
        indicators['Price_Std'] = indicators['Close'].rolling(self.config['mean_reversion_period']).std()
        indicators['Z_Score'] = (indicators['Close'] - indicators['Price_MA']) / indicators['Price_Std']
        
        # Bollinger Band position (0 = lower band, 1 = upper band)
        bb_range = indicators['BB_Upper'] - indicators['BB_Lower']
        indicators['BB_Position'] = (indicators['Close'] - indicators['BB_Lower']) / bb_range
    ...
    def _calculate_oversold_score(self, bb_signal: float, rsi_signal: float, 
        ...
        signals = [max(0, s) for s in [bb_signal, rsi_signal, z_score_signal, price_dev_signal]]
    ...
    def _calculate_overbought_score(self, bb_signal: float, rsi_signal: float,
        ...
        signals = [abs(min(0, s)) for s in [bb_signal, rsi_signal, z_score_signal, price_dev_signal]]
