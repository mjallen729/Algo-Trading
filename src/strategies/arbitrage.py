...
class ArbitrageStrategy(BaseStrategy):
  ...
  def __init__(self, config: Dict = None):
    ...
    logger.info(
      f"ArbitrageStrategy initialized with min spread: {self.min_spread}")
  ...
  def generate_signal(self,
    ...
    inefficiency_signal = self._detect_price_inefficiencies(
      data, current_price)
  ...
  def _detect_price_inefficiencies(self, data: pd.DataFrame, current_price: float) -> float:
    ...
    accel_inefficiency = -np.sign(acceleration) * \
        min(0.5, abs(acceleration) * 1000)
