...
class TFTStrategy(Strategy):
    ...
    def init(self):
        ...
        try:
            ...
            print(f"Error loading model: {e}. Ensure model is trained and saved at {MODEL_PATH}")
        ...
        dummy_index = pd.date_range(start='2020-01-01', periods=TFT_MAX_ENCODER_LENGTH + 50, freq='H')
        dummy_df = pd.DataFrame(dummy_data, index=dummy_index)
        dummy_engineered_df = self.feature_engineer.engineer_features(dummy_df.copy())
        dummy_engineered_df['symbol'] = SYMBOL.split('/')[0] # Add symbol column
        dummy_engineered_df['time_idx'] = dummy_engineered_df.groupby('symbol').cumcount() # Add time_idx
        
        # Ensure target column is present for prepare_data
        if 'close' not in dummy_engineered_df.columns:
            dummy_engineered_df['close'] = dummy_engineered_df['open'] # Placeholder if 'close' is missing after engineering
        
        self.tft_model.prepare_data(dummy_engineered_df, target_col='close', group_id_col='symbol')
    ...
    def next(self):
        ...
        # Engineer features
        engineered_df = self.feature_engineer.engineer_features(current_data_raw.copy())
        ...
        if self.position_open:
            ...
            if current_price < self.stop_loss_price:
                print(f"STOP-LOSS TRIGGERED for {SYMBOL} at price {current_price:.2f} (Stop: {self.stop_loss_price:.2f})")
                self.position.close()
                self.position_open = False
                self.entry_price = 0.0
                self.stop_loss_price = 0.0
            ...
            if signal.endswith("BUY"):
                ...
                if quantity > 0:
                    print(f"Executing {signal} order for {quantity:.6f} of {SYMBOL} at {current_price:.2f}")
