...
class RiskManager:
    ...
    def __init__(self):
        ...
        self.max_position_concentration = config.get('risk.max_position_concentration', 0.25)
        self.max_correlation_exposure = config.get('risk.max_correlation_exposure', 0.60)
    ...
    def evaluate_trade_risk(self, 
        ...
        try:
            ...
            if signal.confidence < config.get('trading.min_confidence_threshold', 0.6):
                self._add_risk_alert(f"Signal confidence too low: {signal.confidence:.3f}")
                return False
            ...
            if not self._check_position_concentration(asset, signal, portfolio_state):
                self._add_risk_alert(f"Position concentration limit exceeded for {asset}")
                return False
            ...
            logger.info(f"Risk check passed for {asset} {signal.signal_type.value} signal")
            return True
    ...
    def _check_position_concentration(self, 
        ...
        if concentration > self.max_position_concentration:
            logger.warning(f"Position concentration {concentration:.2%} exceeds limit {self.max_position_concentration:.2%}")
            return False
    ...
    def _check_volatility_risk(self, 
        ...
        min_confidence_for_volatility = 0.5 + (volatility * 10)  # Scale with volatility
        
        if signal.confidence < min_confidence_for_volatility:
            logger.warning(f"Signal confidence {signal.confidence:.3f} too low for volatility {volatility:.3f}")
            return False
        
        # Regime-based volatility limits
        if regime and "High" in regime and volatility > 0.05:  # 5% daily volatility
            logger.warning(f"Volatility {volatility:.3f} too high for regime {regime}")
            return False
    ...
    def _check_correlation_risk(self, asset: str, portfolio_state: Dict = None) -> bool:
        ...
        if correlation_exposure > self.max_correlation_exposure:
            logger.warning(f"Correlation exposure {correlation_exposure:.2%} exceeds limit {self.max_correlation_exposure:.2%}")
            return False
    ...
    def update_risk_state(self, portfolio_state: Dict):
        ...
        try:
            ...
            if current_drawdown > self.max_portfolio_drawdown * 1.2:  # 20% buffer
                ...
                logger.critical(f"Emergency stop activated: drawdown {current_drawdown:.2%}")
