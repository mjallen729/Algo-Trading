...
class DrawdownManager:
  ...
  def __init__(self):
    ...
    logger.info(
      f"DrawdownManager initialized with max drawdown: {self.max_portfolio_drawdown:.1%}")
  ...
  def update_portfolio_state(self, portfolio_state: Dict):
    ...
    try:
      ...
      if current_value > self.peak_value:
        ...
        if self.in_drawdown:
          recovery_amount = (
            current_value - self._get_drawdown_low()) / self._get_drawdown_low()
  ...
  def check_drawdown_limits(self, portfolio_state: Dict = None) -> bool:
    ...
    if self.current_drawdown > self.max_portfolio_drawdown:
      logger.warning(
        f"Portfolio drawdown {self.current_drawdown:.2%} exceeds limit {self.max_portfolio_drawdown:.2%}")
      return False
  ...
  def _check_circuit_breaker(self):
    ...
    if not self.circuit_breaker_active and self.current_drawdown > self.circuit_breaker_threshold:
      ...
      logger.critical(
        f"Circuit breaker activated at {self.current_drawdown:.2%} drawdown")
  ...
  def _get_drawdown_low(self) -> float:
    ...
    # Look back 100 periods
    drawdown_start_idx = max(0, len(self.value_history) - 100)
    recent_values = [v['value']
                     for v in self.value_history[drawdown_start_idx:]]
  ...
  def get_drawdown_statistics(self) -> Dict:
    ...
    if self.drawdown_history:
      ...
      max_historical_drawdowns = [d['max_drawdown']
                                  for d in self.drawdown_history]
    ...
    if self.in_drawdown and self.drawdown_start_time:
      current_duration = (
        datetime.now() - self.drawdown_start_time).total_seconds() / 3600
  ...
  def get_recovery_progress(self) -> Optional[float]:
    ...
    recovery_progress = (current_value - drawdown_low) / \
        (self.peak_value - drawdown_low)
