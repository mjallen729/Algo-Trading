...
class AdvancedPositionSizer:
    ...
    def calculate_position_size(self, 
        ...
        if self.confidence_scaling and signal.confidence is not None:
            base_size = self._apply_confidence_adjustment(base_size, signal.confidence)
    ...
    def _kelly_fractional_sizing(self, portfolio_value: float) -> float:
        ...
        kelly_pct = (self.win_rate * self.avg_win - loss_rate * self.avg_loss) / self.avg_win
    ...
    def _atr_based_sizing(self, portfolio_value: float, volatility: float) -> float:
        ...
        volatility_adjustment = 0.02 / max(volatility, 0.005)  # Prevent division by zero
    ...
    def get_sizing_metrics(self, signal: TradingSignal, portfolio_value: float) -> Dict:
        ...
        """
        base_size = self.calculate_position_size(signal, signal.symbol, portfolio_value)
    ...
    def suggest_position_adjustments(self, 
        ...
        """
        heat_metrics = self.calculate_portfolio_heat(current_positions, portfolio_value)
