...
class HybridPredictor:
    ...
    def __init__(self):
        ...
        self.transformer_weight = config.get('models.ensemble.transformer_weight', 0.5)
        self.confidence_threshold = config.get('models.ensemble.confidence_threshold', 0.65)
    ...
    def predict(self, 
        ...
        lstm_pred, lstm_conf = self.lstm_model.predict(data, return_confidence=True)
        transformer_pred, transformer_conf = self.transformer_model.predict(data, return_confidence=True)
        ...
        if return_confidence:
            ...
            model_agreement = 1 - abs(lstm_pred - transformer_pred) / max(abs(lstm_pred), abs(transformer_pred), 1e-8)
