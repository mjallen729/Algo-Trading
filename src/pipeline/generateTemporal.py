...
def generateTemporal(df: pd.DataFrame):
  df = preprocessing(df)
  ...
  dataset = TimeSeriesDataSet(
    ...
    time_idx="time_idx",
    target="close",
    # each token/symbol is its own series
    group_ids=["symbol"],
    ...
    time_varying_unknown_reals=["open", "high", "low", "close", "volume usdt", "tradecount"],
    target_normalizer= GroupNormalizer(
      groups=["symbol"], transformation="softplus"
    ).fit_transform(df['close'], df),
    scalers= {
      "open": GroupNormalizer(groups=['symbol'], transformation='softplus'),
      "high": GroupNormalizer(groups=['symbol'], transformation='softplus'),
      "low": GroupNormalizer(groups=['symbol'], transformation='softplus'),
      "close": GroupNormalizer(groups=['symbol'], transformation='softplus'),
      "volume usdt": GroupNormalizer(groups=['symbol'], transformation='softplus'),
      "tradecount": GroupNormalizer(groups=['symbol'], transformation='softplus'),
      "year": GroupNormalizer(groups=['symbol'], transformation='softplus')
    }
  )
...
if __name__ == '__main__':
  p = preprocessing(pd.read_csv('~/Documents/Algo-Trading/data/crypto_processed.csv'))
